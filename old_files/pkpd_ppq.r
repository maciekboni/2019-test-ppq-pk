...
A = read.csv("out.ppq.allpatients.csv")
#A = read.csv("out.allpatients.1000.csv")
...
plot(-1,-1,xlim=c(0,75), ylim=c(-0.1, 4), xlab="hour", ylab="log10 mg PPQ in central compt" )
for ( id in pids ) 
{
    B = subset( A, PID==id ) 
...
plot(0,0,xlim=c(0,75), ylim=c(0, 1600), xlab="hour", ylab="mg PPQ in central compt", yaxt='n' )
for ( id in pids ) 
{
  B = subset( A, PID==id ) 
  #plot( B$HOUR, B$COMP2CONC, pch=19, xlim=c(1,9), ylim=c(-0.1, 0.7) )
...
axis(2, at=seq(100,1600,by=100), las=2) #,  las=2)  # 1 means bottom,2 is left
grid(nx=NULL,ny=16)
...
plot( -1, -1, xlim=c(0,700), ylim=c(-0.1, 3.2), xlab="hour", ylab="mg PPQ in central compt", yaxt='n' )
...
plot( -1, -1, xlim=c(0,700), ylim=c(-0.1, 3.2), xlab="hour", ylab="ng/ml PPQ in central compt", yaxt='n' )
grid(nx=NULL,ny=NULL)
axis(2, at=c(0,0.4771213,1,1.4771213,2,2.4771213,3), labels = c("1","3","10","30","100","300","1000"), las=2) #,  las=2)  # 1 means bottom,2 is left
