...
bool pkdynamics_ppq::stochastic = true;

// constructor
pkdynamics_ppq::pkdynamics_ppq(  )
{
    
    vprms.insert( vprms.begin(), num_params, 0.0 );
    assert( vprms.size()==num_params );
...
//
int pkdynamics_ppq::rhs_ode(double t, const double y[], double f[], void *pkd_object )
{
    pkdynamics_ppq* p = (pkdynamics_ppq*) pkd_object;
...
void pkdynamics_ppq::predict( double t0, double t1 )
{
    //int i;
    gsl_odeiv_system sys = {pkdynamics_ppq::rhs_ode, pkdynamics_ppq::jac, dim, this};   // the fourth argument is a void pointer that you are supossed to use freely; you normally
    ...
    while (t < t1)
    {
        ...
        if( num_doses_given < v_dosing_times.size() )
        {
            ...
            if( t >= v_dosing_times[num_doses_given]  )
            {
                ...
                y0[0] +=  v_dosing_amounts[num_doses_given] * vprms[i_F1_thisdose];
...
void pkdynamics_ppq::initialize( void )
{
...
void pkdynamics_ppq::initialize_params_w_population_means( void )
{
...
void pkdynamics_ppq::initialize_params( void )
{
...
void pkdynamics_ppq::redraw_params_before_newdose()
{
    ...
    double TVF1 = F1COVD;
    double F1 =  vprms[i_F1_indiv] * TVF1 * exp(IOV_rv); // IOV is the between dose variability
    vprms[i_F1_thisdose] = F1;
    ...
    if(pkdynamics_ppq::stochastic)
    {
        ...
        vprms[i_k15] *= exp( - IOV2_rv );
        vprms[i_k56] *= exp( - IOV2_rv );
        vprms[i_k62] *= exp( - IOV2_rv );
    }
...
void pkdynamics_ppq::generate_recommended_dosing_schedule()
{
